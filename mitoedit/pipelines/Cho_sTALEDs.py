...
class ChosTALEDsPipeline(BasePipeline):
    ...
    def process_mtDNA(self, mtDNA_seq, pos):
        ...
        if pos in T_positions:
            ...
            T_positions_adj = self._find_dinucs(
                adjacent_bases, ["CT", "GT", "TG", "TC"], "T"
            )
            A_positions_adj = self._find_dinucs(
                adjacent_bases, ["CA", "GA", "AG", "AC"], "A"
            )
            ...
            logger.info(
                f"The left and right adjacent bases are: {left_adjacent_bases} and {right_adjacent_bases}"
            )
            FLAG = None
            ...
            if left_adjacent_bases[-1] == "A":
                FLAG = True
            for window_source in [
                "sTALED with AD on the left_TALE",
                "sTALED with AD on the right_TALE",
            ]:
                for window_size in range(
                    self.min_window_size, self.max_window_size + 1
                ):  # for window sizes of 14-18bp long --> MAJOR ASSUMPTION!!
                    if window_source == "sTALED with AD on the left_TALE":
                        sTALED_left_T_windows = (
                            self._generate_windows_from_right_sTALED(
                                circular_seq, pos, window_size
                            )
                        )  # generating the TC windows(14bp-18bp)
                        TALES = False
                        for rel_pos, window in enumerate(
                            sTALED_left_T_windows, start=5
                        ):
                            ...
                            bystander_positions = [
                                bystander_pos - ADJACENT_BASES_LEFT_LEN + rel_pos
                                for bystander_pos in (T_positions_adj + A_positions_adj)
                                if bystander_pos != pos
                            ]
                            bystander_positions = [
                                bystander_pos
                                for bystander_pos in bystander_positions
                                if bystander_pos > MIN_LEFT_GAP
                                and bystander_pos <= window_size - MIN_RIGHT_GAP
                            ]
                            marked_window = self._mark_bases(
                                window, rel_pos, bystander_positions
                            )
                            ...
                            if marked_window[int_pos_end + 1] == "T":
                                final_window = self._mark_base_at_position(
                                    marked_window, int_pos_end + 1
                                )
                            elif marked_window[int_pos_ini - 1] == "T":
                                final_window = self._mark_base_at_position(
                                    marked_window, int_pos_ini - 1
                                )
                            else:
                                final_window = marked_window
                            start_position = pos - rel_pos + 1
                            ac_positions = self._find_N_positions(
                                window[4:13], start_position + 3, "AC"
                            )
                            tc_positions = self._find_N_positions(
                                window[4:13], start_position + 3, "TC"
                            )
                            ag_positions = self._find_N_positions(
                                window[4:13], start_position + 3, "AG"
                            )
                            tg_positions = self._find_N_positions(
                                window[4:13], start_position + 3, "TG"
                            )
                            ca_positions = self._find_N_positions(
                                window[3:12], start_position + 3, "CA"
                            )
                            ct_positions = self._find_N_positions(
                                window[3:12], start_position + 3, "CT"
                            )
                            ga_positions = self._find_N_positions(
                                window[3:12], start_position + 3, "GA"
                            )
                            gt_positions = self._find_N_positions(
                                window[3:12], start_position + 3, "GT"
                            )
                            ftc = (
                                tc_positions
                                + tg_positions
                            ...
                            sorted_combined = (
                                sorted(combined_set) if combined_set else []
                            )  # Sort if not empty, else return []
                            all_windows.append(
                                (
                                    self.pipeline_name,
                                    window_source,
                        ...
                        sTALED_right_T_windows = (
                            self._generate_windows_from_left_sTALED(
                                circular_seq, pos, window_size
                            )
                        )  # generating the TC windows(14bp-18bp)
                        TALES = False
                        for rel_pos, window in enumerate(
                            sTALED_right_T_windows, start=5
                        ):
                            ...
                            marked_window = self._mark_bases(
                                window, window_size - rel_pos, bystander_positions
                            )
                            ...
                            ac_positions = self._find_N_positions(
                                window[-12:-3],
                                start_position + window_size - 12 - 1,
                                "AC",
                            )
                            tc_positions = self._find_N_positions(
                                window[-12:-3],
                            ...
                            ca_positions = self._find_N_positions(
                                window[-13:-4], start_position + window_size - 13, "CA"
                            )
                            ct_positions = self._find_N_positions(
                                window[-13:-4], start_position + window_size - 13, "CT"
                            )
                            ga_positions = self._find_N_positions(
                                window[-13:-4], start_position + window_size - 13, "GA"
                            )
                            gt_positions = self._find_N_positions(
                                window[-13:-4], start_position + window_size - 13, "GT"
                            )
                            ftc = (
                                tc_positions
                                + tg_positions
                            ...
                            sorted_combined = (
                                sorted(combined_set) if combined_set else []
                            )  # Sort if not empty, else return [0]
                            all_windows.append(
                                (
                                    self.pipeline_name,
                                    window_source,
            ...
            for window_source in [
                ...
                for window_size in range(14, 19):  # for window sizes of 14-18bp long
                    if window_source == "sTALED with AD on the right_TALE":
                        sTALED_left_A_windows = self._generate_windows_from_left_sTALED(
                            circular_seq, pos, window_size
                        )  # generating the A windows(14bp-18bp)
                        TALES = False
                        for rel_pos, window in enumerate(
                            sTALED_left_A_windows, start=5
                        ):
                            window_desc = f"Position {rel_pos} from the 3' end"
                            ws = f"{window_size}bp"
                        ...
                        TALES = False
                        for rel_pos, window in enumerate(
                            sTALED_right_A_windows, start=5
                        ):
                            window_desc = f"Position {rel_pos} from the 5' end"
                            ws = f"{window_size}bp"
            ...
            raise ValueError(
                f"Base at position {pos} is not in a editable context and cannot be edited by the {self.pipeline_name} pipeline."
            )
